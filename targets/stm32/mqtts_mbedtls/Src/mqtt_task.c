...
void MqttClientPubTask(void const *argument)
{
	...
	while(1)
	{
		if(mqttClient.isconnected)
		{
			...
			MQTTPublish(&mqttClient, brConfig.mfTopic, &message); //publish a message
		}
		osDelay(1000);
	}
}
...
int MqttConnectBroker()
{
	...
	data.username.cstring = brConfig.mfThingId;
	data.password.cstring = brConfig.mfThingPass;
	...
	ret = MQTTSubscribe(&mqttClient, brConfig.mfTopic, QOS0, MqttMessageArrived);
	if(ret != MQTT_SUCCESS)
	{
